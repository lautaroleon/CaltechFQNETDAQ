...
#define THIS_IP            "192.168.155.1"//"192.168.133.52"
#define COMMUNICATION_PORT "10002"             // the port on ZedBoard for communicating with XDAQ
#define STREAMING_PORT     "10002"             // the port on ZedBoard for streaming to XDAQ
#define DESTINATION_IP     "192.168.155.1"  // the IP for the destination of the datastream
#define DESTINATION_PORT   47003              // the port for the destination of the datastream
#define MAXBUFLEN          1492

bool isSampic=true; //false: DT5742
//========================================================================================================================
...
int main(int argc, char **argv){
	...
	std::string commFileName = "/tmp/RunFile.txt";

	if(isSampic) commFileName = "/home/daq/fnal_tb_18_11/SampicControl/command_in.txt";
	...
	while(1)
	{
		...
		if (receiveAndAcknowledge(communicationSocket, messageSender, communicationBuffer) >= 0){
			...
			if (communicationBuffer.substr(0,5) == "START")
			{
				...
				runFile.open(commFileName);
				if(isSampic){
					std::cout<<"Current run number: "<<currentRun<<std::endl;
					runFile << "#1 LOAD_SETUP -FROM .\\Setup\\Setup_MTDnew_3.dat"<<std::endl;
					runFile << "#2 START_RUN -TIME 1 -SAVETO .\\Dummy\\Run"<<currentRun<<".bin"<<std::endl;
					runFile << "#3 START_RUN -TIME 7200 -SAVETO .\\SampicShared\\Run"<<currentRun<<".bin"<<std::endl;

					//runFile << "LOAD_SETUP -FROM .\\SETUP\\Setup_test.dat"<<std::endl;
					//runFile << "START_RUN -HITS 1000 -SAVETO .\\Run_Data\\test_2.bin";
				}
				else{ runFile << "start"; }
				runFile.close();
				...
				std::string command = "echo acuiring data; echo " + currentRun + " > /home/sxie/ETL_Agilent_MSO-X-92004A/Acquisition/ScopeStatus.txt";
