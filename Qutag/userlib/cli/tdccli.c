 ...
 */
/*******************************************************************************/
/* $Id: tdccli.c,v 1.1 2018/02/08 16:28:53 trurl Exp $ */
...
{
  static const char * captions[] = { "1", "2", "3", "4", "5", "6", "7", "8",
  ...
  for ( i = 0; i < TDC_COINC_CHANNELS; ++i ) {
    column += printf( "%s: %d   ", captions[i], coincCnt[i] );
    if ( column > 70 || i+1 == TDC_COINC_CHANNELS ) {
      printf( "\n" );
      column = 0;
    }
  }
...
static void printDevInfo( void )
{
  ...
  Bln32        fHbt = TDC_checkFeatureHbt();
  Bln32        fLft = TDC_checkFeatureLifeTime();
  ...
  printf( "Device found:    Type=%s ID=%d SerialNo=%s\n"
          "                 HBT=%s LifeTime=%s\n"
          "Library version: %.1f\n",
          dTypes[type], id, serNo, yesNo( fHbt ), yesNo( fLft ), TDC_getVersion() );
}
